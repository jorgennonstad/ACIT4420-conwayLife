...
from conwayLife import load_pattern, get_valid_int
from conwayLife import rules

pattern = "patterns/glider.txt"
board = Board(5, 5)
...
def test_count_alive_neighbors_center():
    ...
    assert rules.count_alive_neighbors(board, 1, 1) == 2
...
def test_count_alive_neighbors_corner():
    ...
    assert rules.count_alive_neighbors(board, 0, 0) == 2
...
def test_underpopulation_rule():
    ...
    rules.underpopulation(board, 0, 0, new_grid)
...
def test_reproduction_rule():
    ...
    new_grid = [[cell for cell in row] for row in board.grid]
    rules.reproduction(board, 1, 1, new_grid)
...
def test_survival_rule():
    ...
    rules.survival(board, 1, 1, new_grid)
...
def test_overpopulation_rule():
    ...
    rules.overpopulation(board, 1, 1, new_grid)
